...
function formatStatus(health) {
  ...
  console.log(`  OAuth:     ${statusIcon(health.oauth?.ok)} ${health.oauth?.ok ? 'OK' : 'ERROR'} ${health.oauth?.providers?.length ? `[${health.oauth.providers.join(', ')}]` : '[none]'}`);
  console.log('='.repeat(60));
}
