...
import type { SystemWatcherResult } from '../../packages/ai-types/src/systemWatcher.js';

export interface SystemWatcherInput {
 ...
 */
export async function runSystemWatcher(
  input: SystemWatcherInput
): Promise<SystemWatcherResult> {
...
Be concise but actionable. Focus on high-severity issues first.`;
  ...
  let parsed: SystemWatcherResult;
  try {
    ...
    const { buildInsightAction, inferEntryPointFromInsight, buildPayloadForEntryPoint } = await import('../utils/insightActionBuilder.js');

    // Ensure all insights have required fields and standardized actions
    parsed.insights = parsed.insights.map((insight: any, idx: number) => {
      const baseInsight = {
    ...
    parsed.suggestions = parsed.suggestions.map((suggestion: any, idx: number) => ({
