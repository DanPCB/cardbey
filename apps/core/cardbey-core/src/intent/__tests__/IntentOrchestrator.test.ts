...
describe('IntentOrchestrator', () => {
  ...
  it('starts create_store without store picker when user already has stores', async () => {
    vi.mocked(loadAccountStoreContext).mockResolvedValue({
      accountHasStores: true,
      storeCount: 3,
      stores: [
    ...
    const orchestrator = new IntentOrchestrator();
    const result = await orchestrator.process({
      message: 'create as tore',
      userId: 'user_1',
    });

    expect(result.intent.type).toBe('create_store');
    expect(result.context.status).toBe('ready');
    expect(result.execution.action).toBe('create_store');
    expect(result.execution.parameters?.storeId).toBeUndefined();
  });
});
