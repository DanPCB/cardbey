...
const AU_STATES = /* @__PURE__ */ new Set(["NSW", "VIC", "QLD", "SA", "WA", "TAS", "NT", "ACT"]);
...
const DEMO_FALLBACK_LOCATION_NAMES = new Set(
...
function normalizeCountry(raw, state) {
  const s = trim(raw);
  if (!s) {
...
function shouldRejectSyntheticDemoLocation(locationText, hasUserPrompt) {
...
function extractLocationFromPrompt(prompt) {
  ...
  if (!text) return null;
  const inMatch = /\b(?:in|at|near)\s+([A-Za-z][A-Za-z\s,'-]+?)(?:\s*[,.]|$|\s+(?:vic|nsw|qld|sa|wa|tas|nt|act|australia|au)\b)/i.exec(
    text
...
function buildFromParts(parts, source, confidence) {
  ...
  const displayLocation = formatStoreLocation({
    address: addressLine,
    suburb,
    city,
    state: region,
  ...
  return {
    ...
    countryCode,
    addressLine,
    latitude: Number.isFinite(parts.latitude) ? parts.latitude : null,
    longitude: Number.isFinite(parts.longitude) ? parts.longitude : null,
    source,
...
function resolveCanonicalBusinessLocation(input = {}) {
  ...
  const userLocationText = trim(input.locationText) ?? trim(input.missionContext?.location) ?? extractLocationFromPrompt(trim(input.userPrompt));
    ...
    const built = buildFromParts(
      {
        ...
        region: normalizeStateToken(trim(input.state) ?? trim(input.region) ?? trim(input.operatingRegion)) ?? parsed.region ?? (locality && AU_VIC_LOCALITIES.has(locality.toLowerCase()) ? "VIC" : null) ?? (locality?.toLowerCase() === "melbourne" ? "VIC" : null) ?? (locality?.toLowerCase() === "singapore" ? null : null),
        country: trim(input.country) ?? parsed.country ?? (locality?.toLowerCase() === "melbourne" || locality && AU_VIC_LOCALITIES.has(locality.toLowerCase()) ? "Australia" : locality?.toLowerCase() === "singapore" ? "Singapore" : null),
        countryCode: trim(input.countryCode) ?? parsed.countryCode ?? (locality?.toLowerCase() === "singapore" ? "SG" : locality?.toLowerCase() === "melbourne" ? "AU" : parsed.countryCode),
        latitude: input.latitude,
        longitude: input.longitude
      },
      trim(input.userPrompt) ? "user_prompt" : "user_intake",
    ...
    const built = buildFromParts(
      {
        ...
        region: normalizeStateToken(trim(seed.state) ?? trim(seed.operatingRegion)) ?? parsed.region,
...
function resolveCanonicalLocationFromSeedNormalized(n) {
...
function logLocationCanonicalized(payload) {
  console.log("[LOCATION_CANONICALIZED]", JSON.stringify(payload));
}
function logLocationGenerationMismatch(payload) {
  console.warn("[LOCATION_GENERATION_MISMATCH]", JSON.stringify(payload));
}
function logLocationRepairCandidate(payload) {
