/**
 * Normalize raw facts from any source into a consistent shape, and compute
 * a confidence score. Pure functions — no I/O.
 */
...
export function websiteHost(value: unknown): string | null {
...
export function normalizeOpeningHours(value: unknown): OpeningHours | null {
  ...
  if (Array.isArray(value)) {
    const lines = value.map((v) => cleanString(v)).filter((v): v is string => Boolean(v));
    return lines.length ? { lines } : null;
  }
  if (typeof value === 'object') {
    const obj = value as { lines?: unknown; raw?: unknown; weekday_text?: unknown };
    ...
    const lines = rawLines
      ? rawLines.map((v) => cleanString(v)).filter((v): v is string => Boolean(v))
      : undefined;
...
export function normalizePhotos(value: unknown): string[] {
...
const SOURCE_BASE_CONFIDENCE: Record<DiscoverySource, number> = {
  google_places: 0.7,
  website: 0.55,
  schema_org: 0.6,
  social: 0.4,
...
export function computeConfidence(input: {
  ...
  // A bare name with nothing else is weak.
...
export function normalizeFacts(raw: Record<string, unknown>): NormalizedFacts {
  ...
  if (raw.socialLinks && typeof raw.socialLinks === 'object' && !Array.isArray(raw.socialLinks)) {
    ...
    for (const [k, v] of Object.entries(raw.socialLinks as Record<string, unknown>)) {
      const s = cleanString(v);
      if (s) obj[k] = s;
    }
