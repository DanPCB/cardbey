...
async function repairCompositionTarget(preview, ctx) {
  ...
  // Template merge can reintroduce fabricated social_proof; re-apply the strip after merge.
  const sectionsAfter = preview?.website?.sections;
  if (Array.isArray(sectionsAfter)) {
    const filteredAfter = sectionsAfter.filter((s) => {
      if (s?.type !== 'social_proof') return true;
      const reviews = s?.content?.reviews;
      if (!Array.isArray(reviews)) return true;
      return !reviews.some((r) => FAKE_REVIEW_AUTHORS.has(String(r?.author ?? '').toLowerCase()));
    });
    if (filteredAfter.length !== sectionsAfter.length) {
      preview.website = { ...(preview.website ?? {}), sections: filteredAfter };
      changed = true;
    }
  }
