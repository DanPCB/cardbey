...
import { toPublicPlaybackDto } from './domain.js';
...
/** Legacy alias — still CONNECTING only, never LIVE. */
liveMarketOwnerRoutes.post(
...
liveMarketOwnerRoutes.get(
  '/:storeId/live-sessions/:sessionId/broadcast-capabilities',
  requireAuth,
  requireStoreOwner,
  async (req, res) => {
    try {
...
liveMarketOwnerRoutes.post(
  ...
  rateLimit({
    windowMs: 60_000,
    max: 10,
    keyGenerator: (req) => `live-creds:${req.userId}:${req.params.sessionId}`,
  }),
  async (req, res) => {
    try {
      ...
      // Do not log credentials. Response only.
...
liveMarketAdminRoutes.post('/reconcile-provider', async (req, res) => {
  try {
    const result = await reconcileLiveProviderSessions({
      limit: req.body?.limit,
      maxProviderCalls: req.body?.maxProviderCalls,
    });
    return res.json({ ok: true, ...result });
  } catch (err) {
    return sendError(res, err);
...
/**
 * Canonical public playback DTO. Player only when published + confirmed LIVE + surface flag.
 */
liveMarketPublicRoutes.get('/sessions/:sessionId/playback', requireStorefrontConsume, async (req, res) => {
  try {
    const surface = String(req.query.surface || 'storefront').toLowerCase();
    const consumeOk =
      surface === 'global'
        ? Features.liveMarket.globalPlayerV1
        : Features.liveMarket.storefrontPlayerV1;
    const { getPrismaClient } = await import('../prisma.js');
    const prisma = getPrismaClient();
    const session = await prisma.liveMarketSession.findUnique({
      where: { id: String(req.params.sessionId) },
    });
    if (!session) {
      return res.json({
        ok: true,
        playback: toPublicPlaybackDto({ session: null, consumeEnabled: false }),
      });
    }
    let playerUrl = null;
    let videoId = null;
    if (consumeOk && session.state === 'LIVE' && session.providerExternalRef) {
      const provider = resolveLiveVideoProvider();
      if (typeof provider.getPlaybackInfo === 'function') {
        try {
          const playback = await provider.getPlaybackInfo({
            sessionId: session.id,
            externalRef: session.providerExternalRef,
          });
          playerUrl = playback.playerUrl || null;
    ...
    return res.json({
      ok: true,
      playback: toPublicPlaybackDto({
        session,
        providerConfirmedLive: session.state === 'LIVE',
        playerUrl,
        videoId,
        consumeEnabled: consumeOk,
      }),
    });
  } catch (err) {
    return sendError(res, err);
...
/**
 * Cloudflare Live Input Notifications webhook.
 * Active only when CLOUDFLARE_NOTIFICATIONS_WEBHOOK_AUTH is configured.
 * Mount separately at /api/webhooks/cloudflare/stream-live
 */
export const cloudflareStreamLiveWebhookRoutes = Router({ mergeParams: true });

cloudflareStreamLiveWebhookRoutes.post('/', async (req, res) => {
  try {
    if (!isCloudflareLiveWebhookRouteActive()) {
      return res.status(404).json({ ok: false, error: 'WEBHOOK_INACTIVE' });
    }
    const result = await handleCloudflareLiveInputWebhook({
      headers: req.headers,
      body: req.body,
    });
    return res.status(200).json(result);
  } catch (err) {
    const code = err?.code || 'LIVE_PROVIDER_EVENT_INVALID';
    const status =
