...
import type { AnalysisStatus, SemanticStatus } from '../types.js';
...
export type MarketIntentAdminAnalyzeResult = {
  ok: boolean;
  status: 'READY' | 'PARTIAL' | 'FAILED' | 'DEGRADED';
...
function deriveAdminStatus(
  analysis: MarketIntentAnalysis | null,
  pipelineStatus: 'READY' | 'PARTIAL' | 'FAILED',
): { status: MarketIntentAdminAnalyzeResult['status']; analysisStatus: AnalysisStatus } {
  const analysisStatus = analysis?.analysisStatus ?? 'READY';
  if (analysisStatus === 'DEGRADED') {
    return { status: 'DEGRADED', analysisStatus };
  }
  if (pipelineStatus === 'FAILED') {
    return { status: 'FAILED', analysisStatus: 'FAILED' };
  }
  return { status: pipelineStatus, analysisStatus };
}
...
function withSemanticFields(
  ...
  'status' | 'analysisStatus' | 'semanticStatus' | 'semanticFailureCode'
> {
  const derived = deriveAdminStatus(analysis, pipelineStatus);
  return {
    status: derived.status,
    analysisStatus: derived.analysisStatus,
    semanticStatus: analysis?.semanticStatus,
    semanticFailureCode: analysis?.semanticFailureCode ?? null,
  };
}
...
function buildResult(
  ...
  const status = partial.status ?? 'FAILED';
  return {
    ok: status !== 'FAILED',
    semanticHealth: getMarketIntentSemanticHealth(),
    ...partial,
    status,
  };
}
...
export async function analyzeMarketIntentForAdmin(
  ...
  const semanticFields = withSemanticFields(analysis, 'READY');
