...
export function buildMarketIntentAnalysis(
  ...
  const semanticStatus: SemanticStatus =
    runtime?.semanticStatus ?? (method === 'llm' ? 'AVAILABLE' : 'UNAVAILABLE');
  const analysisStatus: AnalysisStatus =
    runtime?.analysisStatus ?? (method === 'llm' ? 'READY' : 'DEGRADED');
  ...
  return {
    ...
    semanticStatus,
    analysisStatus,
    semanticFailureCode: runtime?.semanticFailureCode ?? null,
    diagnostics: {
      signalId: signal.signalId,
      classification: extracted.classification,
...
export function buildDegradedSemanticAnalysis(
  signal: ExternalMarketSignal,
  failureCode: SemanticFailureCode,
  failureReason: string,
): MarketIntentAnalysis {
  const semanticStatus: SemanticStatus =
    failureCode === 'LLM_PROVIDER_NOT_CONFIGURED' || failureCode === 'LLM_PROVIDER_UNAVAILABLE'
      ? 'UNAVAILABLE'
      : 'FAILED';
  ...
  return {
    ...
    classificationReason: `Semantic extraction failed (${failureCode})`,
