...
Please provide a helpful answer based on the context above. If the context doesn't contain enough information, say so.`;
  ...
  // Answer generation via llmGateway (Phase 0); embeddings remain OpenAI
  const isTest = process.env.NODE_ENV === 'test';
  let answer;
  if (isTest || !hasRagAnswerLlm()) {
    answer = isTest
      ? 'This is a test response. In production, this would be generated by the LLM gateway based on the knowledge base.'
      : 'No LLM provider is configured, so I cannot generate an answer.';
  } else if (Features.llm.useGateway) {
    const result = await llmGateway.complete({
      purpose: 'rag_answer',
      tenantKey: tenantId || 'rag',
      messages: [
    ...
    answer = result.text || 'I apologize, but I couldn\'t generate a response.';
  } else {
