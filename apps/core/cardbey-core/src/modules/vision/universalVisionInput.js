...
export const LoyaltyHintsSchema = z.object({
  punchCountApprox: z.number().optional(),
...
/**
 * Main entrypoint: Analyse vision input and return structured result
 */
export async function analyseVisionInput(req) {
...
function normaliseSam3ToBlocks(sam3Result, ocrText) {
  const blocks = [];
...
function guessBlockKindFromSam3(region) {
...
function deriveMenuHints(blocks, ocrText, uiHints) {
...
function deriveLoyaltyHints(blocks, ocrText) {
...
function derivePromoHints(blocks, ocrText) {
  const textSource =
    ocrText ||
    blocks
      .map((b) => b.text ?? '')
