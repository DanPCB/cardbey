...
export type LLMMenuItem = z.infer<typeof LLMMenuItemSchema>;
export type LLMMenuParseResult = z.infer<typeof LLMMenuParseResultSchema>;

// Input for the parser: what menu.extract knows
export type LLMMenuParserInput = {
  tenantId: string;
  storeId: string | null;
  // OCR text of full image (multi-line)
  ocrText: string;
  // Optional detected item names from the UI grid (e.g. "Flat White", "Latte")
  detectedItems?: string[];
  // Optional language hint (e.g. "en", "vi")
  locale?: string;
  /** Draft / pipeline context so categories match the business vertical */
...
export async function parseMenuWithLLM(
  input: LLMMenuParserInput
): Promise<LLMMenuParseResult> {
  const {
    ocrText,
    detectedItems = [],
    locale = 'en',
