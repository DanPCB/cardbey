 ...
 *
 * Usage (cardbey-core Render shell — rootDir apps/core/cardbey-core):
 *   node scripts/v1-promo-capture-check.mjs --full
 *   (canonical copy lives in apps/core/cardbey-core/scripts/)
 */
...
const CANONICAL_ENTRY =
  '/app?entry=performer&onboarding=1&newStore=1&starter=create_store&source=v1_promo_capture_check';
...
function catalogStats(draftJson) {
  const items =
    draftJson?.catalog?.items ||
    draftJson?.draft?.catalog?.items ||
    [];
...
async function main() {
  ...
  // --- Intake: URL-only (promo alt input) ---
  ...
  // --- Intake: description (promo alt input) ---
  ...
  // --- Full path: grounded research business ---
  if (FULL) {
    const guestFull = `v1-promo-full-${Date.now()}`;
    let intakeBody;
    if (CAPTURE_BUSINESS === 'modern_security_doors') {
      intakeBody = {
    ...
    if (missionId) {
      ...
      allPass = check('structured_store_build completed', poll.ok, poll.failed ? 'failed' : poll.timeout ? 'timeout' : `${poll.elapsed}ms`) && allPass;
      const resolvedGen =
        genRunId ||
      ...
      if (resolvedGen) {
        ...
        allPass = check('Draft available', draft.json?.status === 'ready' || draft.json?.draft?.status === 'ready', draft.json?.status) && allPass;
        allPass = check('Research offerings present', stats.count >= 8, `count=${stats.count}`) && allPass;
        allPass = check('No template catalog fallback', stats.templateCount === 0, `template=${stats.templateCount}`) && allPass;
        allPass =
          check(
