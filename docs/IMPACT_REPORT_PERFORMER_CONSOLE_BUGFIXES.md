...
- **Session-scoped restore could change**: `launch_campaign` missions from a previous browser session will no longer auto-restore (intended). If the session tagging is wrong, it could prevent restoring a legitimately active campaign mission.
...
- **Cross-session persistence**: `localStorage` can rehydrate missions created in prior sessions; for `launch_campaign` this can look like the console “randomly” started a campaign.
...
- **Per-session identity**: `sessionStorage` `cardbey.console.sessionId.v1` and `localStorage` mission snapshot now include session tagging.
...
- **No implicit mission end**: Remove any automatic “new mission” resets during normal execution; only end missions on explicit user action or confirmed 404.
- **Cross-session guard**: Only restore `launch_campaign` from the same browser session (tag snapshots with a per-tab session id).
