...
- **GET /api/mi/missions/:missionId/events 404:** The handler exists in `apps/core/cardbey-core/src/routes/miIntentsRoutes.js`. A 404 in production means the **deployed** app did not have this router mounted at `/api/mi` (e.g. stale deploy).
...
| Item | Status |
|------|--------|
| **Server entry** | `apps/core/cardbey-core/src/server.js` |
...
| File | Change |
|------|--------|
| `apps/core/cardbey-core/src/server.js` | Comments clarifying production-critical mounts for `/api/mi` and `/api/draft-store`. Startup log: `[CORE] Production-critical routes: ...` and `[CORE] DATABASE_URL: set | not set`. |
| `apps/core/cardbey-core/src/test/e2e/m2-unification-closeout.e2e.test.js` | New describe "Production-critical route mounts" with two tests: (1) GET /api/mi/missions/:id/events returns non-404, (2) GET /api/draft-store/:id/summary not Express 404. |
